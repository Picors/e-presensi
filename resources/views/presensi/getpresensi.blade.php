...
@php
    $foto_in = Storage::url('uploads/absensi/'.$prs->foto_in);
    $foto_out = Storage::url('uploads/absensi/'. $prs->foto_out);
@endphp
    <tr>
        ...
        <td>{!! $prs->jam_out !== null ? $prs->jam_out : '<span class="badge bg-danger">Belum Absen</span>' !!}</td>
        <td>
            @if ($prs->jam_out !== null)
        ...
        <td>
            <a href="#" class="btn btn-primary tampilkanmaps" id="{{ $prs->id }}">
...
<script>
    $(function() {
        $(".tampilkanmaps").click(function(e){
            var id = $(this).attr("id");
            $.ajax({
                type: "POST",
                url: '/tampilkanmaps',
            ...
            $("#modal-tampilkanmaps").modal('show');
        });
    });
</script>